...
legado（阅读）之所以好用，靠的是开源社区接力开发、百花齐放的功能迭代。MultiTTS 同样是一款优秀的公益软件，但作者失联、源码丢失后，它的发展就此停滞。本仓库的初衷是接过这一棒：**希望 MultiTTS 也能像 legado 一样百花齐放**——在逆向还原出的可编译工程之上，让社区持续为它添砖加瓦。设想中的功能方向包括但不限于：

- **语音包分组管理**：按语言/场景/引擎给语音包建分组，一键切换整组配置；
- **语音包自定义排序**：支持拖拽排序、置顶常用发音人，摆脱固定的默认顺序；
- **背景音乐分组管理**：BGM 按氛围/书种分组，不同引擎或不同朗读场景绑定不同歌单；
- **更多能力**：欢迎在 Issues 里提出或直接提交 PR——这份可编译的源码工程就是为二次开发准备的土壤。
